...
function requireAdmin(req, res, next) {
    ...
    if (tokenBlacklist.has(token)) return res.status(401).json({ message:'Session expired.' });
    try {
        const decoded = jwt.verify(token, JWT_SECRET);
        if (decoded.role !== 'admin') return res.status(403).json({ message:'Access denied.' });
        req.admin = decoded; req.token = token; next();
...
app.post('/api/bookings', async (req, res) => {
    ...
    try {
        ...
        const [booking] = await Booking.create([{
            carId, qty:Number(qty), customerName:customerName.trim(),
            customerEmail:customerEmail?.trim()||'', customerPhone:customerPhone?.trim()||'',
        ...
        car.stock -= Number(qty); await car.save({ session });
...
app.post('/api/bookings/batch', async (req, res) => {
    ...
    try {
        ...
        for (const item of bookings) {
            ...
            car.stock -= Number(qty); await car.save({ session });
            created.push(booking); eq.push({ booking, carTitle:car.title });
        }
        await session.commitTransaction(); session.endSession();
        for (const {booking,carTitle} of eq) { if (booking.customerEmail) { const {subject,html}=buildSubmittedEmail(booking,carTitle); sendEmail(booking.customerEmail,subject,html); } }
...
app.post('/api/admin/login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id:user._id, role:user.role }, JWT_SECRET, { expiresIn:rememberMe?'30d':'24h' });
...
app.put('/api/admin/bookings/:id/status', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        booking.status = status; await booking.save({ session });
        await session.commitTransaction(); session.endSession();
...
app.post('/api/admin/cars', requireAdmin, async (req, res) => {
    try {
        ...
        const car = await Car.create({ title, description, image, type, stock:Number(stock)||1, dailyRate:Number(dailyRate)||0 });
...
app.put('/api/admin/cars/:id', requireAdmin, async (req, res) => {
    try {
        ...
        if (dailyRate!==undefined)   u.dailyRate   = Math.max(0, Number(dailyRate));
