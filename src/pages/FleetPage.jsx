...
async function uploadImage(file) {
    ...
    if (!cloudName || !uploadPreset) {
        throw new Error('Cloudinary env vars not set. Add VITE_CLOUDINARY_CLOUD_NAME and VITE_CLOUDINARY_UPLOAD_PRESET to your .env');
    }
...
function CarFormModal({ car, onClose, onSaved }) {
    ...
    function handleField(e) {
        ...
        setForm(p => ({ ...p, [name]: name === 'stock' || name === 'dailyRate' ? Number(value) : value }));
    }
                ...
                <form onSubmit={handleSubmit} className="fm-form">
                    ...
                    <div className="fm-row">
                        <div className="fm-group">
                            ...
                            <input name="title" required placeholder="2024 Toyota Camry" value={form.title} onChange={handleField} disabled={busy} />
                        </div>
                        <div className="fm-group">
                            <label>Type *</label>
                    ...
                    <div className="fm-row">
                        <div className="fm-group">
                            ...
                            <input type="number" name="stock" min={0} value={form.stock} onChange={handleField} disabled={busy} />
                        </div>
                        <div className="fm-group">
                            <label>Daily Rate (₱)</label>
                            <input type="number" name="dailyRate" min={0} value={form.dailyRate} onChange={handleField} disabled={busy} />
                        </div>
                    </div>
...
export default function FleetPage() {
    ...
    const fetchCars = useCallback(async () => {
        ...
        setError('');
        try {
            const data = await apiFetch('/api/admin/cars');
        ...
        <div className="fp-root">
            <div className="fp-toolbar">
                <div className="fp-search-wrap">
                    ...
                    <input className="fp-search" placeholder="Search fleet…" value={search} onChange={e => setSearch(e.target.value)} />
                </div>
            ...
            {loading ? (
                ...
                <div className="fp-empty">
                    ...
                    {!search && <button className="fp-add-btn" onClick={() => setModalCar(null)}>Add your first vehicle</button>}
                </div>
            ) : (
                <div className="fp-grid">
                    {filtered.map(car => (
                        <div key={car._id} className="fp-card">
                            ...
                            <div className="fp-card__body">
                                ...
                                <div className="fp-card__meta">
                                    <span className="fp-card__rate">
                                        {car.dailyRate ? `₱${car.dailyRate.toLocaleString()}/day` : 'Rate not set'}
                                    </span>
                                    <div className="fp-stock-ctrl">
                                        <button className="fp-stock-btn" onClick={() => handleStockChange(car, -1)} disabled={car.stock === 0}>−</button>
                                        <span className={`fp-stock-val${car.stock === 0 ? ' fp-stock-val--empty' : ''}`}>{car.stock}</span>
                                        <button className="fp-stock-btn" onClick={() => handleStockChange(car, 1)}>+</button>
                                    </div>
                                </div>
                            </div>
                            <div className="fp-card__actions">
                                <button className="fp-action-btn fp-action-btn--edit" onClick={() => setModalCar(car)}>
            ...
            {modalCar !== undefined && (
                <CarFormModal
                    ...
                    onSaved={() => { setModalCar(undefined); fetchCars(); }}
                />
            )}
        </div>
    );
